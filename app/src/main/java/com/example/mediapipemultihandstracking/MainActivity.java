...
public class MainActivity extends BasicActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        processor.addPacketCallback(
                ...
                (packet) -> {
                    Log.d(TAG, "Datos recibidos de multiples puntos");
                    multiHandLandmarks =
                    ...
                    public void process(Packet packet) {
                        ...
                        try {
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    moveGesture.setText(handGestureMoveCalculator3(normalizedRectsList));
                                }
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
    ...
    private String getMultiHandLandmarksDebugString(List<NormalizedLandmarkList> multiHandLandmarks) {
        if (multiHandLandmarks.isEmpty()) {
            return "No hay datos de mano";
        }
        String multiHandLandmarksStr = "Numero de manos detectadas: " + multiHandLandmarks.size() + "\n";
        int handIndex = 0;
        for (NormalizedLandmarkList landmarks : multiHandLandmarks) {
            multiHandLandmarksStr +=
                    "\t#Señales de manos[" + handIndex + "]: " + landmarks.getLandmarkCount() + "\n";
            int landmarkIndex = 0;
    ...
    private String handGestureCalculator(List<NormalizedLandmarkList> multiHandLandmarks) {
        ...
        List<RectProto.NormalizedRect> normalizedRectList;
        ...
        if (previousXCenter != 0) {
            ...
            if (mouvementDistance > mouvementDistanceThreshold) {
                ...
                if (angle >= -45 && angle < 45) {
                    // return "Movimiento hacia la derecha";

                } else if (angle >= 45 && angle < 135) {
                    // return "Movimiento hacia arriba";
                } else if (angle >= 135 || angle < -135) {
                    // return "Movimiento hacia la izquierda";
                } else if (angle >= -135 && angle < -45) {
                    // return "Movimiento hacia abajo";
                }
            }
        }
    ...
    private String handGestureMoveCalculator2(List<RectProto.NormalizedRect> normalizedRectList) {
    ...
    private String handGestureMoveCalculator3(List<RectProto.NormalizedRect> normalizedRectList) {
        ...
        if (frameCounter && multiHandLandmarks != null) {
            ...
            for (NormalizedLandmarkList landmarks : multiHandLandmarks) {
                ...
                NormalizedLandmark wrist2 = landmarkList.get(2);
                NormalizedLandmark MCP_of_second_finger2 = landmarkList.get(17);
                ...
                double ang_in_radian2 =
                        getAngleABC(MCP_of_second_finger2.getY(), MCP_of_second_finger.getX(),
                                wrist.getY(), wrist.getX(), wrist.getY() + 0.1, wrist.getX());
                int ang_in_degree2 = radianToDegree(ang_in_radian2);
    ...
    private boolean isClose(LandmarkProto.NormalizedLandmark point1, LandmarkProto.NormalizedLandmark point2) {
        double distance = getEuclideanDistanceAB(point1.getX(), point1.getY(), point2.getX(), point2.getY());
        return distance < 0.1;
    }

    private boolean isMajor(LandmarkProto.NormalizedLandmark point1, LandmarkProto.NormalizedLandmark point2) {
        double punto = point1.getY();
        return punto >  point2.getY();
    }
